...
export default function PositionsList({
  ...
  return (
    <div className={`flex flex-col rounded-xl border overflow-hidden shadow-sm flex-1 transition-colors ${
      isDark ? 'border-zinc-800 bg-zinc-950 text-white' : 'border-gray-200/60 bg-white text-black'
    }`}>
      {/* Tab navigators */}
      <div className={`flex h-12 items-center justify-between border-b px-4 ${
        isDark ? 'border-zinc-850 bg-zinc-950' : 'border-gray-100 bg-white'
      }`}>
          ...
          >
            ...
            {activeContracts.length > 0 && (
              <span className={`ml-1.5 flex h-4 min-w-4 items-center justify-center rounded-full px-1 font-mono text-[9px] font-extrabold ${
                isDark ? 'bg-zinc-800 text-white' : 'bg-black text-white'
              }`}>
                  ...
                  return (
                    <div
                      ...
                      className={`rounded-lg border p-3.5 space-y-3.5 shadow-sm transition-all ${
                        isDark ? 'border-zinc-800 bg-zinc-900/60' : 'border-gray-100 bg-gray-50/20 hover:border-gray-100'
                      }`}
                    >
                      {/* Top status */}
                      <div className={`flex items-center justify-between border-b pb-2 ${isDark ? 'border-zinc-850' : 'border-gray-100'}`}>
                        <div>
