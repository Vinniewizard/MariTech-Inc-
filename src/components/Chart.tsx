...
export default function Chart({
  ...
  useEffect(() => {
    ...
    if (localChartType === 'line') {
      ...
      const visibleCandlesCount = Math.min(candles.length, Math.floor(zoomLevel / 2));
    ...
    if (indicatorConfig.sma.enabled) {
      const p = indicatorConfig.sma.period;
      ctx.beginPath();
      ...
      for (let i = 0; i < visibleTicks.length; i++) {
        ...
        if (globalIndex < p) continue;
        let sum = 0;
        for (let j = 0; j < p; j++) sum += ticks[globalIndex - j].price;
        const x = getX(i, visibleTicks.length);
        const y = getY(sum / p);
    ...
    activeContracts.forEach((contract) => {
      ...
      if (contract.type === 'digit-over-under') {
         // Special visual for digits? 
         // Let's draw the target digit on the chart side
    ...
    if (showRSI) {
      ...
      for (let i = 0; i < visibleTicks.length; i++) {
        ...
        const rsiVal = rsiValues[globalIdx] || 50;
  ...
  }, [dimensions, ticks, activeContracts, indicatorConfig, localChartType, zoomLevel, asset, isDark]);
  ...
  return (
    <div className={`relative flex flex-col rounded-xl border p-4 shadow-sm w-full h-[360px] sm:h-[420px] transition-colors ${
      isDark ? 'border-zinc-805 bg-zinc-950 text-white' : 'border-gray-100 bg-white text-black'
    }`}>
      {/* Chart controls toolbar */}
      <div className={`flex flex-wrap items-center justify-between border-b pb-3 mb-3 gap-2 ${
        isDark ? 'border-zinc-850 text-zinc-200' : 'border-gray-50 text-black'
      }`}>
        <div className="flex items-center space-x-3">
          ...
          <div className={`flex items-center space-x-1.5 rounded px-2 py-1 ${
            ...
            <span className={`font-mono text-[9px] px-1 rounded uppercase font-bold ${
              isDark ? 'bg-zinc-800 text-zinc-400' : 'bg-gray-150 text-gray-500'
            }`}>
              {asset.symbol}
            </span>
          </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          <div className="relative">
            <button
              ...
              className={`flex items-center space-x-1 rounded-md px-2.5 py-1 text-xs font-bold cursor-pointer transition-all border ${
                showIndicatorsPanel || Object.values(indicatorConfig).some(i => i.enabled)
                  ...
                  : isDark ? 'bg-zinc-900 text-zinc-400 border-zinc-800 hover:text-white' : 'bg-white text-gray-500 border-gray-200 hover:text-black'
              }`}
            >
              <span>Indicators</span>
              <span className={`text-[10px] rounded px-1 font-mono font-bold ${isDark ? 'bg-zinc-800' : 'bg-gray-100'}`}>
            ...
            {showIndicatorsPanel && (
              <div className={`absolute right-0 mt-1.5 w-48 rounded-md border p-2.5 shadow-xl z-35 ${
                isDark ? 'border-zinc-800 bg-zinc-950 text-white' : 'border-gray-250 bg-white text-black'
              }`}>
                ...
                <div className="space-y-1">
                  <button
                    ...
                    className="flex w-full items-center justify-between rounded px-2 py-1 text-xs hover:bg-gray-50 text-left cursor-pointer"
                  >
          ...
          <div className={`flex space-x-0.5 rounded-lg p-0.5 border ${
            ...
            <button
              ...
              className="rounded p-1 text-gray-400 hover:text-black transition-all cursor-pointer"
              title="Zoom In"
            >
      ...
      <div id="chart-parent" ref={containerRef} className={`relative flex-1 w-full rounded-md overflow-hidden ${
        isDark ? 'bg-zinc-950/20' : 'bg-gray-50/30'
      }`}>
        <canvas
          id="drawing-surface"
          ref={canvasRef}
